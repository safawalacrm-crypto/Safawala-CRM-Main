...
import { createCouponAction, deleteCouponAction, toggleCouponAction, updateCouponAction } from '@/app/coupons/actions';

export type CouponOffer = { id: number; code: string; name: string; discount_type: 'percentage' | 'fixed'; value: number; is_active: boolean; created_at: string };

const BLANK_OFFER: CouponOffer = { id: 0, code: '', name: '', discount_type: 'percentage', value: 0, is_active: true, created_at: '' };

export function CouponsManager({ offers, loadError = '' }: { offers: CouponOffer[]; loadError?: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-5">
      <DashboardHeader
        ...
        backHref="/dashboard"
        actions={<Button size="sm" onClick={openCreate}><Plus className="size-4" /> New Offer</Button>}
      />
      {listError ? <Alert variant="destructive"><AlertDescription>{listError}</AlertDescription></Alert> : null}
      <Card className="border-border shadow-level-1">
        <CardContent className="p-5">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold">All Offers</h2>
              <p className="text-sm text-muted-foreground">{offers.length} total · {active} active</p>
            </div>
            <div className="flex items-center gap-3">
              <Tag className="size-5 text-primary" />
              <Button size="sm" onClick={openCreate}><Plus className="size-4" /> New Offer</Button>
            </div>
          </div>
          <div className="space-y-2">
            {offers.map((offer) => (
              <div key={offer.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-border p-3">
                <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-accent text-primary">
                  {offer.discount_type === 'percentage' ? '%' : '₹'}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium">
                    <code className="mr-2 rounded bg-muted px-1.5 py-0.5 text-xs">{offer.code}</code>
                    {offer.discount_type === 'percentage' ? `${offer.value}% off` : `₹${offer.value} off`}
                  </p>
                  <p className="truncate text-sm text-muted-foreground">{offer.name}</p>
                </div>
                <button
                  type="button"
                ...
                >
                  <span className={`absolute top-1 size-5 rounded-full bg-white shadow transition ${offer.is_active ? 'left-6' : 'left-1'}`} />
                </button>
                <Button variant="ghost" size="icon-sm" onClick={() => openEdit(offer)} aria-label={`Edit ${offer.code}`}>
                  <Pencil className="size-4" />
                </Button>
                <Button variant="ghost" size="icon-sm" onClick={() => remove(offer.id)} aria-label={`Delete ${offer.code}`}>
      ...
      {editing ? (
        <OfferModal offer={editing.id ? editing : null} error={modalError} close={() => setEditing(null)} submit={submit} />
      ) : null}
    </div>
  );
}
        ...
        <CardContent className="p-5 sm:p-6">
          <div className="mb-5 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Customers can redeem this offer during booking.</p>
            </div>
          ...
          <form action={submit} className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-3 rounded-lg border border-border p-3 text-sm sm:col-span-2">
              <input type="checkbox" name="is_active" defaultChecked={offer?.is_active ?? true} className="size-4 accent-primary" />
              <span>
                <span className="block font-medium">Active</span>
                <span className="text-muted-foreground">Customers can redeem this offer immediately</span>
              </span>
            </label>
            <div className="flex justify-end gap-2 sm:col-span-2">
              <Button type="button" variant="outline" onClick={close}>Cancel</Button>
              <Button type="submit"><Check className="size-4" /> {offer ? 'Save Changes' : 'Create Offer'}</Button>
            </div>
          </form>
        </CardContent>
      </Card>
