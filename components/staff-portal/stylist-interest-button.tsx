...
export function StylistInterestButton({ jobId }: { jobId: string }) {
  const [state, action, pending] = useActionState(expressInterestAction, INITIAL_STATE);
  return (
    <div className="space-y-2">
      <form action={action}>
        <input type="hidden" name="jobId" value={jobId} />
        <Button type="submit" size="sm" disabled={pending || state.saved}>
          {pending ? <LoaderCircle className="animate-spin" /> : state.saved ? <Check /> : null}
          {pending ? 'Saving…' : state.saved ? 'Interested' : "I'm Interested"}
        </Button>
      </form>
      {state.error ? <p className="max-w-64 text-xs text-destructive" role="alert">{state.error}</p> : null}
    </div>
  );
}
