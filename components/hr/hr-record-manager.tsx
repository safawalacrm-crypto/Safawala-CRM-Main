...
export function HrRecordManager({ module, staff }: { module: string; staff: Staff[] }) {
  ...
  function submit(form: HTMLFormElement) { const data = new FormData(form); start(async () => { const supabase = createClient(); const { data: auth } = await supabase.auth.getUser(); const payload: Record<string, unknown> = { owner_id: auth.user?.id, staff_id: Number(data.get('staff_id')) }; config.forEach((f) => { const value = String(data.get(f.name) ?? ''); payload[f.name] = !value ? null : f.type === 'number' ? Number(value) : value; }); const { error } = await supabase.from(module === 'work-orders' ? 'hr_work_orders' : `hr_${module}`).insert(payload); setMessage(error?.message ?? 'Record saved successfully.'); if (!error) { setOpen(false); form.reset(); window.location.reload(); } }); }
  return <Card><CardContent className="flex items-center justify-between gap-4 p-5"><div><p className="font-semibold">Add {module === 'work-orders' ? 'work order' : module.replace('-', ' ')}</p><p className="text-sm text-muted-foreground">Create a record for an existing staff member.</p>{message && <p className="mt-2 text-sm text-[#70481c]">{message}</p>}</div><Button onClick={() => setOpen(true)}>Add record</Button>{open && <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4" onMouseDown={(e) => e.target === e.currentTarget && setOpen(false)}><Card className="w-full max-w-lg"><CardContent className="space-y-4 p-6"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold">Add record</h2><button onClick={() => setOpen(false)} aria-label="Close">×</button></div><form onSubmit={(e) => { e.preventDefault(); submit(e.currentTarget); }} className="space-y-3"><label className="block text-sm font-medium">Staff<select name="staff_id" required className="mt-1 h-10 w-full rounded-lg border px-3">{staff.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label>{config.map((field) => <label key={field.name} className="block text-sm font-medium">{field.label}<input name={field.name} type={field.type ?? 'text'} required className="mt-1 h-10 w-full rounded-lg border px-3" /></label>)}<Button type="submit" disabled={pending} className="w-full">{pending ? 'Saving…' : 'Save record'}</Button></form></CardContent></Card></div>}</CardContent></Card>;
}
