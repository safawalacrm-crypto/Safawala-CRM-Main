...
export default async function HrDashboardPage() {
  ...
  return <DashboardShell email={auth.user.email ?? 'Safawala user'}><div className="mx-auto max-w-[1280px] space-y-6"><DashboardHeader title="HR & Staff" subtitle="Manage your team, attendance, payroll, and employee records" /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Card><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total staff</p><p className="mt-2 text-3xl font-semibold">{count ?? 0}</p><p className="mt-1 text-sm text-muted-foreground">Registered employee records</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Admin control</p><p className="mt-2 text-lg font-semibold">Access & roles</p><p className="mt-1 text-sm text-muted-foreground">Managed from Staff Directory</p></CardContent></Card></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{cards.map(({ href, label, description, icon: Icon, live }) => { const content = <Card className={`h-full ${live ? 'transition hover:-translate-y-0.5 hover:border-[#d6b98d] hover:shadow-level-2' : 'opacity-75'}`}><CardContent className="flex h-full gap-4 p-5"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#f5ead8] text-[#70481c]"><Icon className="size-5" /></span><span><strong className="block">{label}</strong><span className="mt-1 block text-sm text-muted-foreground">{description}</span><span className={`mt-3 inline-block text-xs font-medium ${live ? 'text-[#70481c]' : 'text-muted-foreground'}`}>{live ? 'Open module →' : 'Planned module'}</span></span></CardContent></Card>; return live ? <Link key={label} href={href} className="group">{content}</Link> : <div key={label}>{content}</div>; })}</div></div></DashboardShell>;
}
